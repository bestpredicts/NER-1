...
parser.add_argument('--num_fold', type=int, default=5, help="一共有几折")
parser.add_argument('--fold_id', type=int, default=0, help="cross-valid索引")
...
parser.add_argument('--threshold', type=int, default=1, help="投票阈值")
...
def fold_ensemble(params, num_fold, mode, num_samples, threshold):
    ...
    with open(params.params_path.parent / 'fusion_submit.txt', 'w', encoding='utf-8') as f_sub:
        # convert dict to json
        json_data = json.dumps(submit, indent=4, ensure_ascii=False)
        f_sub.write(json_data)
