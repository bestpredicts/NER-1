    ...
    with open(DATA_DIR / f'{mode}.data', 'w', encoding='utf-8') as f, \
        ...
        for idx, c in enumerate(content):
            if c[2][0] != 'NaN' and c[3][0] != 'NaN':
                ...
                f_bio.write('{}\n'.format(' '.join(text)))
                # 抽取实体位置
                # entities = list(findall(p=c[3], s=text))
                for tp_id, tp in enumerate(c[2]):
                    result.append({
                        ...
                        'span_position': [f"{loc[0]};{loc[1]}" for entity in c[3][tp_id]
                ...
                if mode == 'train':
                    # add other labels
                    other_labels = list(set(STR2IO.values()).difference(set([STR2IO[tp] for tp in c[2]])))
                    random.shuffle(other_labels)
                    for label in other_labels[:3]:
                        result.append({
                            'id': c[0],
                            'context': text,
                            'entity_label': label,
                            'query': IO2QUERY[label],
                            'span_position': [],
                            'start_position': [],
                            'end_position': []
                        })
