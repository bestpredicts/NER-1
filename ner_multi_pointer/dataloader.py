...
from dataloader_utils import read_pointer_ner_examples, convert_examples_to_features
...
class MRCNERDataLoader(object):
    ...
    def convert_examples_to_features(self, data_sign):
        ...
        if os.path.exists(cache_path) and self.data_cache:
            ...
            if data_sign == "train":
                examples = read_pointer_ner_examples(os.path.join(self.data_dir, "train.data"))
            elif data_sign == "val":
                examples = read_pointer_ner_examples(os.path.join(self.data_dir, "val.data"))
            elif data_sign == "test":
                examples = read_pointer_ner_examples(os.path.join(self.data_dir, "test.data"))
            else:
    ...
    def get_dataloader(self, data_sign="train"):
        ...
        start_label = torch.tensor([f.start_position for f in features], dtype=torch.long)
        end_label = torch.tensor([f.end_position for f in features], dtype=torch.long)
        dataset = TensorDataset(input_ids, input_mask, start_label, end_label)
...
if __name__ == '__main__':
    ...
    params = Params()
    datalodaer = MRCNERDataLoader(params)
    f = datalodaer.convert_examples_to_features(data_sign='val')
    print(f[0].start_position[42])
    print(f[0].end_position[43])
