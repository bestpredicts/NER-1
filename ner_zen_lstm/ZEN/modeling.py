...
from ZEN.file_utils import cached_path, WEIGHTS_NAME, CONFIG_NAME
    ...
    def forward(self, hidden_states, ngram_hidden_states, ngram_position_matrix, attention_mask,
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i < num_hidden_ngram_layers:
                ngram_hidden_states = self.word_layers[i](ngram_hidden_states, ngram_attention_mask, head_mask[i])  # (bs, max_ngram, hidden_size)
                if self.output_attentions:
...
class ZenModel(ZenPreTrainedModel):
    ...
    Inputs:
        ...
        `input_ngram_ids`: input_ids of ngrams （batch_size, max_ngram）.
    ...
    def forward(self, input_ids,
        ...
        ngram_embedding_output = self.word_embeddings(input_ngram_ids, ngram_token_type_ids)  # (bs, max_ngram, hidden_size)
...
class ZenForPreTraining(ZenPreTrainedModel):
    ...
    def __init__(self, config, output_attentions=True, keep_multihead_output=False):
